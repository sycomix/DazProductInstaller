...
EXCLUDE_CONDITIONS = {
    ".cs": [lambda filename: all(x not in filename for x in SUFFIXES)]
}
...
def compare_message(astr: list[str], file_path: str) -> bool:
    ...
    return abs(len(comment_msg) - len(msg_words)) <= 3
def get_byte_order(str_) -> tuple[int] | None:
    try:
        byte_order = tuple(str_[:3])
        for bo in BYTE_ORDERS:
            next = any(a != ord(b) for a, b in zip(bo, byte_order))
            if next:
                continue
            return bo
    except:
...
def get_comment_message(file_path: str) -> str:
    ...
    stringBuilder = "".join(
        LANGUAGE_TO_COMMENT[ext.lower()] + line + "\n"
...
def simulate(file_path: str) -> bool:
    ...
    if two_lines and not compare_message(two_lines, file_path):
    ...
    return "y" in input("Do you wish to proceed? (Y/N) ").lower()
...
def handle_dir(path: str) -> bool:
    ...
    for file in files:
        if (
            SIMULATE
            and not simulate(file)
            or not SIMULATE
            and not execute(file)
        ):
            return False
    for dir in dirs:
